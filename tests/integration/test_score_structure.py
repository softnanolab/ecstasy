...
def dataset():
    ...
    if not d.index.exists():
        pytest.skip(f"{DATASET} index not present at {d.index}")
    return d


@pytest.fixture(scope="module")
def entry(dataset) -> Entry:
    """The first dimer in the split that carries full-atom GT."""
    for e in dataset.entries():
        if len(e.sequences) == 2 and dataset.native_bundle(e.id) is not None:
            return e
    pytest.skip("no entry with full-atom ground truth")
